...
- Cascade vision parity: `look_at_screen` attaches the captured screen as a high-detail `input_image` to the current GPT-5.5 turn (PNG/JPEG, 15 MB cap, untrusted-content guidance), detached after its turn so screenshots never ride future full-history requests; `take_screenshot` is excluded from `control_computer` so pixels flow only through vision.
- `capture_screen_context` stores a screen capture as a first-class `screenshot` context artifact (file copied into the state directory, path/mime payload) that `submit_task` accepts via `attachmentIds`, closing the "no valid attachment ID for screenshots" gap; coding agents receive the absolute path through the standard attachment rendering.
- The cascade tool chain cap was raised from 4 to 24 consecutive rounds — a runaway brake rather than a budget, since interactive computer control legitimately chains many rounds; barge-in and mute remain the human stop.
